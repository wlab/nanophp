	...
	var tinymce = {
		...
		_init : function() {
			...
			for (i=0; i<nl.length; i++) {
				if (v = nl[i].href) {
					// Host only value like http://project.com or http://project.com:8008
					if (/^https?:\/\/[^\/]+$/.test(v))
						v += '/';
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		init : function() {
			...
			// IE has a bug where it remove project absolute urls to relative ones if this is specified
